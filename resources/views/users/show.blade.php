...
<body>
    @extends('layouts.app') @section('title', 'Detail Pengguna: ' . $user->name) @section('content')
        <div class="container">
            ...
            <div class="detail-group">
                <strong>Kantor Cabang:</strong> <span>{{ $user->branch->name ?? '-' }}</span>
            </div>
            <div class="detail-group">
                <strong>No. Telp:</strong> <span>{{ $user->phone_number ?? '-' }}</span>
            </div>
            <div class="detail-group">
                <strong>Dibuat Pada:</strong> <span>{{ $user->created_at->format('d M Y H:i') }}</span>
            </div>
            <div class="detail-group">
                <strong>Diperbarui Pada:</strong> <span>{{ $user->updated_at->format('d M Y H:i') }}</span>
            </div>

            <div class="action-links">
                <a href="{{ route('users.edit', $user->id) }}" class="edit-button">Edit Pengguna</a>
                <form action="{{ route('users.destroy', $user->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="delete-button" onclick="return confirm('Anda yakin ingin menghapus pengguna ini?')">Hapus Pengguna</button>
                </form>
            </div>
